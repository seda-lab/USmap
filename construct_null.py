...
def construct_null(csvfilename, confilename, nullfilename, labelfilename, graphfilename, fdistfilename, users=False, base=5, dist=True):

	if base <= 0.0: dist = False;
	...
	with open(csvfilename, 'r') as csvfile:
		...
		for row in spamreader:
			if row[0] in G.nodes():
				node_info[ row[0] ] = {"N":float(row[nid]), "coord":np.array([float(row[-2]), float(row[-1])]) }		
	...
	if dist:			
		...
		for i in ordered_nodes:
			for j in ordered_nodes:
				if G.has_edge( str(i), str(j) ):
					Anorm += G[str(i)][str(j)]["weight"]
				Nnorm += node_info[str(i)]["N"] * node_info[str(j)]["N"]
		...
		with open(nullfilename, 'w') as outfile:
			for i,u in enumerate(ordered_nodes):
				for j,v in enumerate(ordered_nodes[i:]):
					outfile.write("{} {} {}\n".format(labels[u],labels[v],  node_info[str(u)]["N"] * node_info[str(v)]["N"] * norm) )
