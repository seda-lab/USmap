...
class Quality {
 public:
  ...
  Graph & h; // network to compute communities for
  int size; // nummber of nodes in the network and size of all vectors
  string name;
  
  vector<int> n2c; // community to which each node belongs
 Quality(Graph &gr, Graph &hr, const std::string& n):g(gr),h(hr),size(g.nb_nodes),name(n){}
  virtual ~Quality();
  
  // remove the node from its current community with which it has dnodecomm links
  virtual void remove(int node, int comm, long double dnodecomm, long double dnodecomm_h)=0;
  
  // insert the node in comm with which it shares dnodecomm links
  virtual void insert(int node, int comm, long double dnodecomm, long double dnodecomm_h)=0;
