...
char *filename_null = NULL;
char *filename_nullw = NULL;
...
usage(char *prog_name, const char *more) {
  ...
  cerr << "\tid = 10\t -> user input null model" << endl;
  ...
  cerr << "-n file\tread the null model" << endl;
  cerr << "-x file\tread the null model weights" << endl;
...
parse_args(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
	...
	break;
      case 'n':
        filename_null = argv[i+1];
	i++;
	break;
	  case 'x':
        filename_nullw = argv[i+1];
	i++;
	break;
      case 'q':
...
void
init_quality(Graph *g, Graph *h, unsigned short nbc) {
...
main(int argc, char **argv) {
  ...
  if(id_qual != 10){
	htype = type;
	filename_null = filename;  
	filename_nullw = filename_w;  
  }
  Graph h(filename_null, filename_nullw, htype);
  init_quality(&g, &h, nb_calls);
  nb_calls++;
  if (verbose){
	cerr << "g graph " << g.nb_nodes << " " << g.nb_links << " " << g.total_weight << endl;
	cerr << "h graph " << h.nb_nodes << " " << h.nb_links << " " << h.total_weight << endl;
  }  
  if (verbose)
  ...
  int level = 0;
  cerr << "starting_qual " << quality << endl;
  ...
  do {
    ...
    g = c.partition2graph_binary(true);
    if( id_qual == 10 ){
		h = c.partition2graph_binary(false);
	} else {
		h = g;
	}
    init_quality(&g, &h, nb_calls);
